...
/**
 * PM HELPER V4.1 - DUAL-MODE AUTHENTICATION
 *
...
class PMHelper {
  constructor(config = {}) {
    ...
    this.role = 'pm';
    this.tokenFile = path.join(__dirname, '.pm-token.json');
  ...
  async viewReport(taskId) {
    console.log(`\n📄 Viewing report ${taskId}...\n`);
    return await this.request('GET', `/agent/pm/task/${taskId}/view`);
  }
...
if (require.main === module) {
  ...
  (async () => {
    try {
      console.log('🚀 PM Helper V4.1 - Authentication Demo\n');
      console.log('Orchestrator URL:', pm.orchestratorUrl);
      console.log('');
      ...
      if (pm.mode === 'internet') {
        const tokenInfo = pm.getTokenInfo();
        if (tokenInfo) {
